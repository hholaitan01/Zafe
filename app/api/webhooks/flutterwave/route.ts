...
export async function POST(req: Request): Promise<Response> {
  ...
  // successful with Flutterwave, then that the buyer paid the right currency and
  // amount — BEFORE consuming the event id. A transient verify failure returns
  // 409 without burning the event, so a retry can still fund the deal; claiming
  // first would strand a real payment as a "duplicate" forever.
