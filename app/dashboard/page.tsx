...
export default function Page() {
  ...
  useEffect(() => {
    ...
    listDeals()
      .then((deals) => {
        if (!alive) return;
        setData({ deals: deals.length ? deals.map(card).join("") : `<div style="padding:18px; text-align:center; color:#6d6d74; font-size:13px;">No escrows yet. Tap New Escrow to start one.</div>` });
      })
      .catch(() => {});
    return () => {
      alive = false;
