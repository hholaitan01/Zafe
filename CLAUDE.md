...
story, gold `#A16207` sparingly, Plus Jakarta Sans, soft depth on a `#F8FAFC` canvas.
...
- **Always open a PR and monitor it.** After a batch of work, open a pull request
  (or update the open one for the branch) — don't wait to be asked. Then watch it:
  respond to review comments and drive CI to green until it merges or closes. If
  the branch's PR is already merged, restart the branch from the default branch and
  open a new PR for the follow-up work. Standing owner instruction.
- Keep secrets out of the repo (`.env.local` is git-ignored). Never put keys in
  chat or commits.
- **No "Generated by Claude Code" attribution.** Never add "Generated by Claude
  Code", "Generated with Claude Code", or any similar Claude/AI "generated-by"
  footer to PR titles or bodies, code comments, docs, or any pushed or shared
  artifact (including generated documents and PDFs). Standing, repeated owner
  instruction.
